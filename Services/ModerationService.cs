...
namespace Chika.Services
{
    public class ModerationService
    {
        ...
        public async Task SaveConfigurationAsync()
        {
            ...
            if (!File.Exists(Path.Combine(Directory.GetCurrentDirectory(), "Configurations", "moderation.json")))
                File.Create(Path.Combine(Directory.GetCurrentDirectory(), "Configurations", "moderation.json")).Dispose();
            using (var configStream = new FileStream(Path.Combine(Directory.GetCurrentDirectory(), "Configurations", "moderation.json"), FileMode.Truncate))
            {
        ...
        {
            if (!File.Exists(Path.Combine(Directory.GetCurrentDirectory(), "Configurations", "moderation.json"))) return;
            using (var configStream = new FileStream(Path.Combine(Directory.GetCurrentDirectory(), "Configurations", "moderation.json"), FileMode.Truncate))
            {
